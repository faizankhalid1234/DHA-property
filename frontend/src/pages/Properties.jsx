    ...
    <div className="pt-28 pb-20 page-bg min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <p className="text-muted">Browse premium plots, houses, and commercial properties</p>
        </motion.div>

        <div className="glass-card p-6 mb-10">
          <div className="flex items-center gap-2 mb-4 text-heading font-semibold">
            <Filter size={18} /> Advanced Search
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <div className="relative md:col-span-2">
              <Search className="absolute left-3 top-3.5 text-faint" size={18} />
              <input
                type="text"
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((i) => (
              <div key={i} className="glass-card h-96 animate-pulse bg-gray-200/50 dark:bg-slate-700/50" />
            ))}
          </div>
        ) : properties.length === 0 ? (
